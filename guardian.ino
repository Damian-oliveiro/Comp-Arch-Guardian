...
const unsigned long NO_MOTION_TIME = 20000; // 20 seconds
...
bool isMoving() {
  ...
  float delta = abs(x - lastX) + abs(y - lastY) + abs(z - lastZ);
  ...
  // Require 3 stable consecutive movement readings
  return (movementCount >= 3);
}
...
void loop() {
  ...
  // Track motion (but DO NOT return)
  if (isMoving()) {
    lastMotionTime = now;
  }

  // Check no motion for 20 seconds
  if (now - lastMotionTime > NO_MOTION_TIME) {
    if (now - lastAlertTime > ALERT_COOLDOWN) {
      sendTelegramAlert("No motion detected for 20 seconds!");
      lastAlertTime = now;
      lastMotionTime = now;
    }
  }
